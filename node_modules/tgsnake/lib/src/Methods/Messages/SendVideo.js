...
async function sendVideo(client, chatId, file, more = {}) {
    Logger_js_1.Logger.debug(`exec: send_video chat ${typeof chatId} (${chatId}) ${(0, Utilities_js_1.parseArgObjAsStr)({
        file,
    })} ${(0, Utilities_js_1.parseArgObjAsStr)(more)}`);
    var { disableNotification, replyToMessageId, replyToStoryId, messageThreadId, scheduleDate, sendAsChannel, protectContent, replyMarkup, entities, parseMode, caption, invertMedia, filename, progress, thumbnail, mimetype, forceDocument, hasSpoiler, duration, width, height, supportsStreaming, muted, preloadPrefixSize, } = more;
    if (typeof file === 'string') {
        ...
        if (/^http/i.test(file)) {
            ...
            if (media.fileType === platform_node_js_1.FileType.VIDEO) {
